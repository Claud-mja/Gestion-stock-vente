...
import { RavitaillementCreate, RavitaillementInfo, RavitaillementList } from '../../models/ravitaillement.model';
...
export class RavitaillementService {
  ...
  getRavs() : Observable<Paginated<RavitaillementList>> {
    return this.http.get(`${environment.baseStockUrl}/ravitaillement`).pipe(
      map((response) => {
        
        return (response as ApiResponse).data as Paginated<RavitaillementList>
      })
    );
  }

  getRav(id : number) : Observable<RavitaillementInfo> {
